#!/bin/bash
set -e

BACKEND_PORT=${BACKEND_PORT:-5000}
STREAMLIT_PORT=${PORT:-8080}

echo "Starting backend (gunicorn) on port ${BACKEND_PORT}..."
gunicorn -w 2 -b 0.0.0.0:${BACKEND_PORT} backend.backend_api:app &

# Wait a few seconds for backend to come up
sleep 3

# Health check
if curl -s http://127.0.0.1:${BACKEND_PORT}/status >/dev/null; then
  echo "Backend is up on port ${BACKEND_PORT}"
else
  echo "Backend failed to start on port ${BACKEND_PORT}" >&2
  exit 1
fi

echo "Starting Streamlit on port ${STREAMLIT_PORT}..."
export STREAMLIT_SERVER_HEADLESS=true
export STREAMLIT_SERVER_PORT=${STREAMLIT_PORT}
export STREAMLIT_SERVER_ENABLE_CORS=false
# Backend URL for frontend to call inside the container
export BACKEND_URL=${BACKEND_URL:-http://127.0.0.1:${BACKEND_PORT}}

streamlit run frontend/main_ui.py --server.port ${STREAMLIT_PORT} --server.address 0.0.0.0
